...
export async function joinProject(projectId: string) {
  return json<{ status: string }>(
    await apiFetch(`${BASE}/projects/${projectId}/join`, { method: "POST" })
  );
}
...
export async function fetchDocuments(projectId?: string) {
  ...
  return json<DocumentOut[]>(await apiFetch(`${BASE}/documents/?${params.toString()}`));
}
...
export async function createCode(label: string, colour: string, projectId: string, definition?: string) {
  return json<CodeOut>(
    await apiFetch(`${BASE}/codes/`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ label, colour, project_id: projectId, ...(definition ? { definition } : {}) }),
    })
  );
}
...
export async function fetchCodes(projectId?: string) {
  const params = new URLSearchParams();
  if (projectId) params.set("project_id", projectId);
  return json<CodeOut[]>(await apiFetch(`${BASE}/codes/?${params.toString()}`));
}
...
export async function batchCreateSegments(items: {
  document_id: string;
  text: string;
  start_index: number;
  end_index: number;
...
export async function fetchSegments(documentId?: string) {
  ...
  return json<SegmentOut[]>(await apiFetch(`${BASE}/segments/?${params.toString()}`));
}
...
export async function fetchAnalyses(projectId?: string) {
  const params = new URLSearchParams();
  if (projectId) params.set("project_id", projectId);
  return json<AnalysisOut[]>(await apiFetch(`${BASE}/segments/analyses?${params.toString()}`));
}

export async function fetchAlerts(unreadOnly = true) {
  const params = new URLSearchParams({ unread_only: String(unreadOnly) });
  return json<AlertOut[]>(await apiFetch(`${BASE}/segments/alerts?${params.toString()}`));
}

export async function fetchCodeSegments(codeId: string) {
  return json<SegmentOut[]>(await apiFetch(`${BASE}/codes/${codeId}/segments`));
}
...
export async function sendChatMessage(message: string, projectId: string, conversationId?: string | null) {
  return json<{ conversation_id: string; status: string }>(
    await apiFetch(`${BASE}/chat/`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ message, project_id: projectId, ...(conversationId ? { conversation_id: conversationId } : {}) }),
    })
  );
}

export async function fetchChatHistory(conversationId: string) {
  return json<ChatMessageOut[]>(await apiFetch(`${BASE}/chat/history/${conversationId}`));
}

export async function fetchConversations(projectId: string) {
  const params = new URLSearchParams({ project_id: projectId });
  return json<ConversationPreview[]>(await apiFetch(`${BASE}/chat/conversations?${params.toString()}`));
}
...
export async function fetchEditHistory(projectId: string, params?: { document_id?: string; entity_type?: string; limit?: number; offset?: number }) {
  ...
  return json<EditEventOut[]>(await apiFetch(`${BASE}/projects/${projectId}/edit-history?${qs.toString()}`));
}

// -- Project Settings

export async function fetchProjectSettings(projectId: string) {
  return json<ProjectSettings>(await apiFetch(`${BASE}/projects/${projectId}/settings`));
}
...
export async function fetchThresholdDefinitions() {
  return json<ThresholdDefinition[]>(await apiFetch(`${BASE}/projects/threshold-definitions`));
}

// -- Visualisations

export async function fetchVisOverview(projectId: string) {
  return json<OverviewData>(await apiFetch(`${BASE}/projects/${projectId}/vis/overview`));
}

export async function fetchVisFacets(projectId: string, codeId?: string | null) {
  const params = new URLSearchParams();
  if (codeId) params.set("code_id", codeId);
  return json<{ facets: FacetData[] }>(await apiFetch(`${BASE}/projects/${projectId}/vis/facets?${params.toString()}`));
}

export async function fetchVisConsistency(projectId: string, codeId?: string | null) {
  const params = new URLSearchParams();
  if (codeId) params.set("code_id", codeId);
  return json<ConsistencyData>(await apiFetch(`${BASE}/projects/${projectId}/vis/consistency?${params.toString()}`));
}

export async function fetchVisOverlap(projectId: string) {
  return json<CodeOverlapData>(await apiFetch(`${BASE}/projects/${projectId}/vis/overlap`));
}

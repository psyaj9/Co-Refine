...
export default function LeftPanel() {
  const {
    ...
    codes, activeCodeId, setActiveCode, addCode, deleteCode,
          ...
          {showNewProject && (
            <div className="flex items-center gap-1 mb-2">
              ...
              <button
                ...
                className="rounded p-1.5 text-surface-400 hover:text-surface-600"
              >
                <X size={14} />
              </button>
            </div>
  ...
  // ── Project active view: contextual reveal layout ──────────────────────────
  ...
  return (
    <div className="flex flex-col h-full panel-bg overflow-hidden">
      ...
      {/* Documents section — collapsible strip */}
      <div className="flex-shrink-0 border-b panel-border">
        <button
          onClick={() => setDocsOpen((v) => !v)}
          className="w-full flex items-center justify-between px-3 py-1.5 text-2xs font-semibold uppercase tracking-wider text-surface-500 dark:text-surface-400 hover:bg-surface-50 dark:hover:bg-surface-800 transition"
        >
          <span className="flex items-center gap-1.5">
            <FileText size={10} />
            Documents
            <span className="ml-1 font-mono normal-case text-surface-400">({documents.length})</span>
          </span>
          <div className="flex items-center gap-1.5">
            <span
              onClick={(e) => { e.stopPropagation(); setShowUploadPage(true); }}
              role="button"
              className="text-brand-500 hover:text-brand-700 dark:hover:text-brand-300 p-0.5 rounded hover:bg-brand-50 dark:hover:bg-brand-900/20"
        ...
        {docsOpen && (
          <div className="max-h-44 overflow-y-auto thin-scrollbar">
            <DocumentsContent
      ...
      {/* Codes section — fills remaining space */}
      <div className="flex flex-col flex-1 overflow-hidden min-h-0">
        ...
        <CodesContent
          ...
          editingDefCodeId={editingDefCodeId}
          ...
          setNewCodeDefinition={setNewCodeDefinition}
...
interface DocumentsContentProps {
  filteredDocs: ReturnType<typeof useStore.getState>["documents"];
  documents: ReturnType<typeof useStore.getState>["documents"];
  ...
  handleDeleteDocument: (e: React.MouseEvent, id: string) => void;
}
      ...
      <ul className="px-2 pb-2 space-y-0.5">
        {filteredDocs.length === 0 && (
          <li className="text-center py-8">
            <FileText size={24} className="mx-auto text-surface-300 dark:text-surface-600 mb-2" />
            <p className="text-xs text-surface-400 italic">No documents yet</p>
            <button
              onClick={() => setShowUploadPage(true)}
              className="mt-3 rounded bg-brand-600 px-3 py-1.5 text-xs font-medium text-white hover:bg-brand-700"
            >
              Upload Document
            </button>
          </li>
        ...
        {filteredDocs.map((doc) => (
          <li
            ...
            className={cn(
              "group cursor-pointer rounded px-2 py-1.5 text-xs flex items-center gap-1.5 transition",
...
// ── Codes (Codebook) tab content ────────────────────────────────────────────

interface CodesContentProps {
  filteredCodes: ReturnType<typeof useStore.getState>["codes"];
  codes: ReturnType<typeof useStore.getState>["codes"];
  ...
  editingDefCodeId: string | null;
  setEditingDefCodeId: (id: string | null) => void;
  editDefText: string;
  setEditDefText: (t: string) => void;
  newCodeLabel: string;
  setNewCodeLabel: (v: string) => void;
  newCodeDefinition: string;
  setNewCodeDefinition: (v: string) => void;
  handleAddCode: () => void;
...
function CodesContent({
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <ul className="flex-1 overflow-auto px-2 pb-2 thin-scrollbar space-y-0.5">
            ...
            <li key={code.id}>
              <div
                onClick={() => setActiveCode(code.id)}
                  onDoubleClick={() => { loadRetrievedSegments(code.id); setExpandedCodeId(code.id); }}
                  className={cn(
                  ...
                  title="Click to select · Double-click to load segments"
              >
                <button
                  onClick={(e) => {
                    e.stopPropagation();
              ...
              {isExpanded && (
                <div className="ml-6 mr-1 mt-1 mb-2 space-y-1.5">
                  <div className="rounded bg-surface-50 dark:bg-surface-800 p-2">
                    ...
                    {isEditingDef ? (
                      ...
                      <p className="text-2xs text-surface-500 dark:text-surface-400 line-clamp-3">
                        {code.definition || (
                          <span className="italic text-surface-400">No definition — click ✏ to add</span>
                        )}
                      </p>
                    )}
                  </div>
                  ...
                  <div className="flex gap-1">
                    <button
                      onClick={() => { loadRetrievedSegments(code.id); setExpandedCodeId(code.id); }}
                      className={cn(
                        "flex items-center gap-1 rounded px-2 py-1 text-2xs font-medium transition",
                    ...
                    <button
                      ...
                      className="flex items-center gap-1 rounded px-2 py-1 text-2xs font-medium bg-surface-100 dark:bg-surface-700 text-surface-600 dark:text-surface-300 hover:bg-brand-50 dark:hover:bg-brand-900/20 hover:text-brand-600 transition"
                  ...
                  {/* Inline segments — only shown when this code's segments are loaded */}
                  {retrievedCodeId === code.id && (

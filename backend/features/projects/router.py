...
def create_project_endpoint(
    ...
    created = create_new_project(db, body.name, owner_id=current_user.id)
...
def join_project(
    ...
    existing = get_membership(db, project_id, current_user.id)
    if not existing:
...
def get_project(
    ...
    if not project:
        raise HTTPException(status_code=404, detail="Project not found")
    if not get_membership(db, project_id, current_user.id):
